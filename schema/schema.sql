-- ============================================================
-- 学创部知识库 · MySQL Schema v1.1
-- 字符集：utf8mb4 / utf8mb4_unicode_ci
-- 版本要求：MySQL 8.0+
-- 部署：团队服务器（内网）
-- 说明：所有标识符统一使用反引号，避免与 MySQL 关键字冲突
--       （如 POSITION 为关键字，职务表命名为 `member_position`）
-- ============================================================
...
CREATE TABLE `member` (
    ...
    `grade`         VARCHAR(10)     DEFAULT NULL COMMENT '年级，如2026级',
    `major`         VARCHAR(100)    DEFAULT NULL COMMENT '专业',
    `class_no`      VARCHAR(50)     DEFAULT NULL COMMENT '班级（敏感）',
    `birthday`      DATE            DEFAULT NULL COMMENT '生日（敏感）',
    `join_term`     VARCHAR(20)     DEFAULT NULL COMMENT '入部学年，如2026-2027',
    ...
    `member_type`   VARCHAR(20)     NOT NULL DEFAULT '本部门' COMMENT '本部门/老师/嘉宾/外部',
...
CREATE TABLE `activity_series` (
    ...
    `is_official` TINYINT         NOT NULL DEFAULT 0 COMMENT '是否官方活动',
    `created_at`  DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`  DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
...
CREATE TABLE `participation` (
    ...
    `exp_ids`      JSON            DEFAULT NULL COMMENT '关联经验id数组（向量库的exp-xxx）',
    `created_at`   DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at`   DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
...
-- ------------------------------------------------------------
-- 5. 职务表（多职务 / 换届）
-- 命名 `member_position`：POSITION 是 MySQL 关键字，避免歧义
-- ------------------------------------------------------------
