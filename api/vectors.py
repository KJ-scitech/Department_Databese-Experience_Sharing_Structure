...
def update_tags(exp_id: str, tags: list) -> dict | None:
    """只改标签，不重新生成向量。给 K8 AI 打标用。
    绝对不要删了重加——会换 exp_id，作者/活动/参与记录全断。"""
    result = get(exp_id)
    if result is None:
        return None
    result["metadata"]["tags"] = ",".join(str(t) for t in tags)
    get_collection().update(ids=[exp_id], metadatas=[result["metadata"]])
    return result
