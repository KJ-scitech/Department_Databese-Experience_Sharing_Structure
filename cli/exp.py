# -*- coding: utf-8 -*-
"""经验知识库的 CLI 子命令：search / get / list / add / delete / patch-tags。
...
def register(sub, parents) -> None:
    ...
    # §4.9 契约：只改标签（K8 AI 打标用）。
    # 服务端 PATCH /api/experience/{exp_id} 只接受 tags，别的字段不开放。
    # 绝对不要用「删了重加」来改标签——会换 exp_id，作者/活动/参与记录全断。
    p = actions.add_parser(
        "patch-tags",
        parents=parents,
        help="只改标签（K8 AI 打标用，§4.9 契约）",
    )
    p.add_argument("exp_id", help="形如 exp-1")
    p.add_argument("--tags", required=True, help="新标签，逗号分隔，如 投影仪,设备")
    p.set_defaults(run=run_patch_tags)
...
def run_patch_tags(args, api: client.Client) -> client.Result:
    tags = [t.strip() for t in args.tags.split(",") if t.strip()]
    if not tags:
        raise client.ApiError("bad_input", "标签是空的，没东西可改", client.EXIT_USAGE)
    item = api.patch(f"/api/experience/{args.exp_id}", {"tags": tags})
    return client.Result(data=item, message=f"已更新 {item.get('exp_id')} 的标签")
